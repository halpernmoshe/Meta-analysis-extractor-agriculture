...
Native-control biochar comparison, with the same categorical pairing and analysis structure as
the primary paired test.
"""
import csv, glob, math, os, re, statistics
from collections import defaultdict
# ---------------------------------------------------------------------------
# VARIANT (new analysis, 2026-08-19): native-control biochar comparison.
# NATIVE_BIOCHAR_CONTROL=True disables the absolute-control harmonisation, so each
# side uses the control it natively recorded. The two sides then estimate DIFFERENT
# quantities and the equivalence test is expected to fail by construction.
# Every other line is identical to the as-submitted scope_aware_paired_tost.py.
# ---------------------------------------------------------------------------
NATIVE_BIOCHAR_CONTROL = True
...
for ds in ["Boldorini","Biochar","Hui","Loladze","Li2022"]:
    ...
    if ds=="Biochar" and not NATIVE_BIOCHAR_CONTROL:
