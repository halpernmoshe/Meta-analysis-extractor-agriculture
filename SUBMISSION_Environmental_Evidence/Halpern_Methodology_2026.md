...
*Correspondence:* Moshe Halpern, hmoshe@volcani.agri.gov.il. Institute of Soil, Water and Environmental Sciences, Agricultural Research Organization --- Volcani Center, Rishon LeZion 7505101, Israel.
...
**Conclusions:** A single AI agent achieves statistical equivalence with human-extracted meta-analysis data across five independent agricultural datasets. The approach reduces extraction cost by approximately three orders of magnitude while maintaining accuracy sufficient for aggregate meta-analytic pooling.

**Keywords:** meta-analysis, data extraction, large language models, artificial intelligence, validation, equivalence testing, environmental evidence synthesis, agriculture
...
Several systems have demonstrated stronger performance under specific conditions. Cao et al. [13] developed OttoSR, achieving 93.1% accuracy on structured clinical data extraction across 7 Cochrane reviews. Gartlehner et al. [14] found AI-assisted extraction achieved 91.0% accuracy versus 89.0% for human-only extraction, with a median time saving of 41 minutes per study. Poser et al. [15] showed that three-model consensus reduced extraction errors to 1.48% for clinical reports. Khan et al. [16] demonstrated that LLMs in a collaborative two-reviewer workflow outperformed individual LLMs. However, virtually all published validation studies operate on clinical or medical data -- randomized controlled trials, Cochrane reviews, and CONSORT-formatted reports. The sole exception outside medicine is Gougherty and Clipp [9] in ecology, which found LLMs poor at quantitative extraction (23.8% accuracy using GPT-3.5). The agricultural domain, with its complex factorial designs and non-standardized reporting, remains largely untested.
...
The extraction system is deployed through three interfaces to accommodate different use cases:
...
Single-paper extraction required approximately 2--5 minutes of processing time and cost approximately $0.08--0.15 per paper using the Claude API. The complete extraction of all five datasets (~179 papers) required approximately 6--8 hours of unattended processing time at a total API cost of approximately $15--25. By comparison, manual extraction of 179 papers at an estimated 2--8 hours per paper [3] would require roughly 360--1,430 hours of trained researcher time -- a reduction of approximately two orders of magnitude in labor. These estimates exclude the time for prompt development and validation, which was a one-time fixed cost amortized across all datasets.

## 2.2 Validation Datasets

Five published meta-analyses served as reference standards for validation. No agent parameters were tuned against any reference standard. All five datasets were processed independently, with the agent receiving only the dataset-specific instruction (treatment definition, control definition, outcome variable).
...
For example, the Hui 2025 dataset (mean absolute effect ~50%) receives a margin of approximately +-10 pp, while the Loladze 2014 dataset (mean absolute effect ~12%) receives a margin of approximately +-2.5 pp. We report cluster-robust standard errors using the CR2 bias-corrected sandwich estimator with Satterthwaite degrees of freedom [24].
...
**Bland-Altman analysis.** Mean difference and 95% limits of agreement [25], with proportional bias assessed via regression of the difference on the mean.
...
**Loladze 2014.** The most structurally challenging dataset, yet LLM-driven alignment revealed strong extraction quality. Pearson r = 0.984, MAE = 1.36 pp, direction agreement = 95.0%. Of 45 matched papers, 27 achieved Excellent-tier agreement (MAE < 5 pp), 14 Good, 4 Fair, and zero Poor. Aggregate effect: reference = -7.83%, agent = -7.82%, difference = 0.01 pp. This dataset features complex factorial designs (CO2 x cultivar x ozone x nitrogen), diverse exposure systems (FACE, OTC, growth chambers), and multiple tissues and developmental stages. Direction agreement is element-specific: Fe and Mn increase under elevated CO2, which is biologically correct, not an extraction error.
...
[FIGURE 1: Scatter plots of agent-extracted versus reference-standard effect sizes for all five datasets. Panels A--E arranged in order of decreasing correlation. Dashed lines indicate identity (y = x). Each panel annotated with r, N, and MAE. The figure should show the tight clustering around the identity line for Hui 2025, Li 2022, and Li 2024 (biochar), all five panels showing tight agreement, with Loladze 2014 exhibiting slightly wider scatter reflecting small effect sizes and factorial complexity.]
...
The source-type labeling reveals a clear accuracy gradient by data provenance, analyzed on the Li 2024 (biochar) dataset where both table and figure sources are well represented.
...
To assess extraction stability, the agent was run twice independently on three datasets with no shared state, cached outputs, or intermediate results between runs.
...
A fixed absolute equivalence margin cannot serve across datasets with order-of-magnitude differences in effect size. A +-3 pp margin is approximately one-quarter of the mean absolute effect for Loladze 2014 (~12%) but less than one-tenth of the mean effect for Hui 2025 (~50%). The proportional approach -- setting the margin at +-20% of the mean absolute effect -- asks a consistent question across datasets: is the extraction bias small relative to the phenomenon?

The 20% threshold is motivated by practical considerations: under inverse-variance pooling, a systematic bias of 20% of the mean effect would shift the pooled estimate by 20% of itself. For most meta-analytic applications, this would not change the qualitative conclusion (e.g., whether an effect is positive or negative, or whether it exceeds a policy-relevant threshold). We encourage future validation studies to adopt proportional margins and to justify their choice of threshold explicitly.
...
The agent was run using Claude Code Pro at a flat subscription fee of $200/month. At this pricing, the entire 137-paper validation was completed within a single subscription period, making the effective per-paper cost negligible at scale. By comparison, manual extraction costs an estimated $60--240/paper for single extraction and $120--480/paper for dual extraction (2--8 hours x $30/hour) [3].

For a hypothetical 500-paper agricultural meta-analysis, manual dual extraction would require an estimated $60,000--240,000 and 6--12 months of researcher time; the agent would complete the same task in under a week. This cost reduction makes living meta-analyses [26] economically viable: periodic re-extraction of an entire literature can be performed at trivial marginal cost.
...
| Marshall et al. 2016 | Clinical | ML | Risk-of-bias automation | No | No |
...
Three distinctions are noteworthy. First, this study is the first to apply formal equivalence testing to AI-extracted continuous data. Second, it validates across five independent datasets rather than one. Third, it provides run-to-run reproducibility assessment on 1,231 observations across 95 papers.
...
This protocol operationalizes the Cochrane/Campbell/JBI/CEE joint position statement on AI use in evidence synthesis [29].
...
The Cochrane, Campbell, JBI, and CEE joint position statement on AI use in evidence synthesis [29] requires disclosure of AI tool use and human oversight. We endorse this position.
...
All AI-assisted extraction should be disclosed per the Cochrane/Campbell/JBI/CEE position statement on AI use in evidence synthesis [29].
...
2. Shojania KG, Sampson M, Ansari MT, Ji J, Doucette S, Moher D. How quickly do systematic reviews go out of date? A survival analysis. *Ann Intern Med*. 2007;147:224-33.
...
4. Buscemi N, Hartling L, Vandermeer B, Tjosvold L, Klassen TP. Single data extraction generated more errors than double data extraction in systematic reviews. *J Clin Epidemiol*. 2006;59:697-703.
5. Higgins JPT, Thomas J, Chandler J, et al., editors. Cochrane handbook for systematic reviews of interventions (version 6.4). Cochrane; 2023.
...
7. Topp CFE, et al. AgroEcoList: a checklist to improve reporting of ecological research in agronomy. *PLoS One*. 2023;18:e0285478.
8. Nakagawa S, et al. A robust and readily implementable method for the meta-analysis of response ratios with and without missing standard deviations. *Ecol Lett*. 2023;26:232-44.
9. Gougherty AV, Clipp HL. Testing the reliability of an AI-based large language model to extract ecological information from the scientific literature. *npj Biodivers*. 2024;3:13.
10. Helms Andersen T, et al. Using AI tools as second reviewers in systematic reviews. *Cochrane Evid Synth Methods*. 2025.
11. Jansen T, et al. Data extraction by generative artificial intelligence. *Psychol Bull*. 2025;151:1280-306.
...
13. Cao X, et al. OttoSR: automation of systematic reviews with large language models. *medRxiv*. 2025. https://doi.org/10.1101/2025.01.15.25320588
...
16. Khan MA, Ayub U, Naqvi SAA, et al. Collaborative large language models for automated data extraction in living systematic reviews. *JAMIA*. 2025;32:638-47.
17. Hui X, Luo L, Chen Y, Palta JA, Wang Z. Zinc agronomic biofortification in wheat and its drivers: a global meta-analysis. *Nat Commun*. 2025;16:3913.
18. Li J, Van Gerrewey T, Geelen D. A meta-analysis of biostimulant yield effectiveness in field trials. *Front Plant Sci*. 2022;13:836702.
19. Li S, et al. Biochar increases crop yield: a meta-analysis. figshare. 2024. https://doi.org/10.6084/m9.figshare.c.6622375.v1
20. Boldorini E, Lucchi A, Tamburini G. Predator-mediated biocontrol of crop arthropod pests and their damage: a global meta-analysis. *Proc R Soc B*. 2024;291:20232522.
21. Loladze I. Hidden shift of the ionome of plants exposed to elevated CO2 depletes minerals at the base of human nutrition. *eLife*. 2014;3:e02245.
...
23. Lin LI-K. A concordance correlation coefficient to evaluate reproducibility. *Biometrics*. 1989;45:255-68.
24. Pustejovsky JE, Tipton E. Small-sample methods for cluster-robust variance estimation and hypothesis testing in fixed effects models. *J Bus Econ Stat*. 2018;36:672-83.
25. Bland JM, Altman DG. Statistical methods for assessing agreement between two methods of clinical measurement. *Lancet*. 1986;327:307-10.
26. Elliott JH, Turner T, Clavisi O, et al. Living systematic reviews: an emerging opportunity to narrow the evidence-practice gap. *PLoS Med*. 2014;11:e1001603.
27. Marshall IJ, Kuiper J, Wallace BC. RobotReviewer: evaluation of a system for automatically assessing bias in clinical trials. *J Am Med Inform Assoc*. 2016;23:193-201.
28. Tendal B, Higgins JPT, Juni P, Hrobjartsson A, Gotzsche PC. Multiplicity of data in trial reports and the reliability of meta-analyses: empirical study. *BMJ*. 2009;339:b3128.
29. Flemyng E, et al. Position statement on the use of artificial intelligence in the production of evidence syntheses. *Cochrane Database Syst Rev*. 2025.
...
MH conceived the study, developed the extraction software and validation framework, performed all extractions and analyses, and wrote the manuscript.

### Availability of data and materials
All extraction code, configuration files, validation scripts, and pre-computed outputs are publicly available at https://github.com/halpernmoshe/Meta-analysis-extractor-agriculture. The repository includes complete LLM extraction prompts and alignment prompts used for all five datasets, enabling full replication. Reference-standard datasets are from published meta-analyses: Loladze (2014), Hui et al. (2025), Li et al. (2022), Li et al. (2024), and Boldorini et al. (2024).  Source PDFs cannot be redistributed due to publisher copyright.
