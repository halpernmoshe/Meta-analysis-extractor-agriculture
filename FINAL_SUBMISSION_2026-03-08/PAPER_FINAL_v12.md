...
**Background:** Data extraction is the primary bottleneck in meta-analysis, requiring 2–8 hours per paper with single-extractor error rates of 17.7%. Large language models achieve >90% accuracy on categorical variables but only 26–36% on continuous outcomes. No study has validated a single AI agent — reading PDFs directly without domain-specific prompt engineering — against multiple independent ground-truth datasets with formal equivalence testing.

**Methods:** A general-purpose AI agent (Claude Opus 4.6) extracted quantitative data from source PDFs across three published plant science meta-analyses: Loladze 2014 (CO₂ effects on plant minerals, 46 papers), Hui 2023 (zinc biofortification, 25 papers), and Li 2022 (biostimulants, 16 high-confidence papers). The agent was developed with no access to any reference standard; Hui and Li are fully independent holdouts, while Loladze is independent for the agent but the validation framework was developed iteratively with Loladze data (see Section 3.2). We validated against published reference standards (1,184 observations, 87 papers), assessed run-to-run reproducibility (1,231 observations, 95 papers), and compared agent output against an independently developed multi-model consensus pipeline without ground truth (1,889 observations, 100 papers). Agreement was evaluated using intraclass correlation coefficients (ICC(3,1)), cluster-robust two one-sided tests (TOST) for equivalence, and Bland-Altman analysis.

**Results:** Agent extraction achieved ICC(3,1) = 0.966 (Li, 68 obs), 0.942 (Hui, 461 obs), and 0.845 (Loladze, 655 obs). Cluster-robust TOST confirmed equivalence at ±3 pp for all datasets (p ≤ 0.047). Cohen's d ranged from 0.016 to 0.103 (all negligible). Aggregate effects were reproduced within 0.22–1.09 pp. Ground-truth-free comparison with a structurally independent pipeline yielded r > 0.93 across all three datasets (1,889 obs, 100 papers). An error taxonomy showed that alignment ambiguity — which factorial sub-condition to extract — dominated over true reading errors (97% vs. 3% of diagnosable discrepancies). Cost: ~$0.15/paper.

**Conclusions:** A single AI agent achieves sufficient accuracy for aggregate meta-analytic pooling across diverse agricultural domains. Cross-method agreement on 1,889 observations without ground truth provides a circularity-free validation framework. At three orders of magnitude less cost than manual extraction, the approach makes previously infeasible agricultural meta-analyses tractable.

**Keywords:** meta-analysis, data extraction, large language models, AI agent, agriculture, plant science, validation, equivalence testing, TOST
...
**Primary metric.** ICC(3,1) (two-way mixed, absolute agreement) following Koo and Li (2016), who classify 0.50–0.75 as "moderate," 0.75–0.90 as "good," and >0.90 as "excellent." Secondary metrics: Pearson r, mean absolute error (MAE), and direction agreement (whether extracted and reference effect sizes share the same sign).

**Equivalence testing.** Two one-sided tests (TOST) with both naive and cluster-robust standard errors. Cluster-robust SEs use the CR1 sandwich estimator, clustering by paper, with t(K−1) degrees of freedom, where K is the number of papers. We selected the ±3 pp margin as the primary equivalence threshold based on practical meta-analytic considerations: in inverse-variance pooled meta-analysis, a ±3 pp shift in individual effect sizes produces negligible change in the pooled estimate when averaging over dozens of observations. This margin corresponds to approximately 25% of the mean Hui effect, 50% of the mean Li effect, and a larger fraction of the mean Loladze effect; consequently, we also report results at tighter margins (±1, ±2 pp) to enable readers to apply their own thresholds.
...
Design effects: Loladze = 2.66, Hui = 4.09, Li = 0.49. The Hui design effect reflects substantial within-paper correlation (many observations per paper from the same zinc biofortification study), which inflates the effective standard error. Naive TOST passes at ±2 pp for all three datasets (all p < 0.014); the cluster adjustment substantially inflates standard errors for Hui and Loladze, providing a more conservative and appropriate test.
...
A single general-purpose AI agent achieves sufficient accuracy for aggregate meta-analytic pooling across three diverse plant science datasets. ICC ranged from 0.845 (complex factorial) to 0.966 (standardized tabular), with all Cohen's d < 0.20 (negligible bias) and cluster-robust TOST equivalence confirmed at ±3 pp for all datasets. These results substantially exceed published benchmarks for continuous data extraction by LLMs (Table 6).

These results are not an artifact of overfitting: no agent parameters were tuned against any reference standard, and Hui and Li are fully independent holdouts for both the agent and the validation framework. Loladze is independent for the agent but the matching protocol was developed with Loladze data (see Section 3.2); the convergent evidence from the two fully independent datasets (Hui ICC = 0.942, Li ICC = 0.966) provides the strongest guard against circularity. A structurally independent multi-model pipeline (Appendix A) converged on the same values (r > 0.93, 1,889 observations) without ground truth, and the pipeline's own formal statistics against the same reference standards are consistent with the agent's (Appendix A, Table A1).

## 5.2 Cost-Effectiveness

**Table 7. Cost comparison of extraction methods.**
...
At ~$0.15/paper and approximately two minutes of processing time, the agent enables meta-analyses that would be prohibitively expensive to conduct manually. The cost for the entire 92-paper validation (~$14) is less than the labor cost of manually extracting a single paper. For a hypothetical 500-paper agricultural meta-analysis, the agent would cost approximately $75 and complete extraction in under a day, compared to an estimated $30,000–120,000 and 3–12 months for manual dual extraction.
...
**Table 6. Comparison with published LLM extraction systems.**
...
Our results substantially exceed published benchmarks for continuous numerical extraction. The difference likely reflects the use of a frontier model with its full 200K-token context window, allowing the agent to read entire papers rather than isolated excerpts. The structured JSON output schema also constrains extraction format, reducing hallucination. However, direct comparison is limited by differences in domain complexity and evaluation methodology.
...
**Figure 2.** Bland-Altman plots showing the difference between agent-extracted and reference-standard effect sizes plotted against their mean. Panel A: Loladze 2014. Panel B: Hui 2023. Panel C: Li 2022. Horizontal solid lines show mean difference; dashed lines show 95% limits of agreement. The regression line for proportional bias is shown for Loladze (r = −0.150, p < 0.001).

**Figure 3.** Scatter plots of agent-extracted vs. pipeline-extracted effect sizes (ground-truth-free comparison). Panel A: Loladze 2014 (r = 0.933, N = 1,205). Panel B: Hui 2023 (r = 0.971, N = 185). Panel C: Li 2022 (r = 0.994, N = 499). Dashed lines indicate identity (y = x). Neither method was calibrated against the other.

**Figure 4.** Per-paper MAE distribution across all three datasets, showing the spread of extraction accuracy at the paper level.
