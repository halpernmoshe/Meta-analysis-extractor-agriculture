...
> Halpern, M. (2026). "AI-Extracted Effect Sizes Achieve Statistical Equivalence with Published Meta-Analysis Data Across Three Plant Science Datasets." *Research Synthesis Methods*.
...
### Task A: Agent Extraction -- Loladze 2014 (Table 1)

```bash
python validate_agent_extraction.py
```

**Expected output**: Metadata-resolved subset: r=0.891, MAE=3.4pp, 447 observations, ICC=0.890. Combined: r=0.887, 650 observations across 46 papers. Tier breakdown: 37 Excellent, 2 Good, 6 Fair, 1 Poor.

### Task B: Agent Extraction -- Hui 2025 (Table 1)
...
### Task C: Agent Extraction -- Li 2022 (Table 1)
...
**Expected output**: Raw match: 166 obs, r=0.446. Harmonized HIGH tier: r=0.968, MAE=1.6pp, 68 observations across 16 papers. ICC(3,1)=0.966.

### Task D: Agent-Pipeline Agreement (Table 4, GT-free)
...
### Task E: Replication Agreement (Table 3)

```bash
python validate_replication.py
```

**Expected output**: Loladze r=0.816 (665 obs), Hui r=0.946 (362 obs), Li r=0.849 (204 obs). Total: 1,231 matched observations across 95 papers. Results in `output/replication_agreement.json`.

### Task F: Formal Statistics (Table 2: TOST; Section 4.2.2: Cohen's d, Bland-Altman)
...
**Expected output**: JSON reports in `output/agent_formal_stats/`. ICC(3,1) = 0.890 (Loladze metadata-resolved), 0.942 (Hui), 0.966 (Li). Cohen's d: 0.054 (Loladze), 0.006 (Hui), 0.065 (Li).
...
### Task F2: CR2 Bias-Corrected TOST (Table 2, Supplementary Table S1)

```bash
python supplementary_cr2_tost.py
```

**Expected output**: `output/supplementary_table_s1_cr2.md` and `.json`. Key results: Loladze metadata-resolved +/-2pp PASS, Hui +/-2pp FAIL (p=0.099), Hui +/-3pp PASS (p=0.002), Li +/-2pp PASS.
...
- `fig5_error_taxonomy.png` -- Error taxonomy
...
### Task H: Pipeline Validation (Appendix A, Table A1)
...
**Expected output**: Pipeline results as reported in Appendix A, Table A1: Hui r=0.999/MAE=0.43pp, Li r=0.951/MAE=2.30pp, Loladze r=0.886/MAE=4.36pp.

### Automated Reproducibility Check

To verify all paper-claimed values automatically:

```bash
python run_all_validations.py
```

This runs all validation scripts in sequence, compares computed values against paper-claimed values for every table, and reports PASS/FAIL for each claim. Results are written to `output/reproducibility_check.json`. Use `--verbose` for detailed output, or `--skip-run` to check existing JSON outputs without re-running scripts.
